...
TEST_F(ArrayTopNTest, transformBasic) {
  ...
  auto expected =
      makeArrayVectorFromJson<int32_t>({"[3, 2]", "[6, 5]", "[9, 8]"});
  auto result =
      evaluate("array_top_n(c0, BIGINT '2', x -> x)", makeRowVector({input}));
  assertEqualVectors(expected, result);
}
...
TEST_F(ArrayTopNTest, transformAbsoluteValue) {
  ...
  auto result = evaluate(
      "array_top_n(c0, BIGINT '3', x -> abs(x))", makeRowVector({input}));
  assertEqualVectors(expected, result);
}
...
TEST_F(ArrayTopNTest, transformNegation) {
  ...
  auto result = evaluate(
      "array_top_n(c0, BIGINT '2', x -> 0 - x)", makeRowVector({input}));
  assertEqualVectors(expected, result);
}
...
TEST_F(ArrayTopNTest, transformWithNulls) {
  ...
  auto result =
      evaluate("array_top_n(c0, BIGINT '3', x -> x)", makeRowVector({input}));
  assertEqualVectors(expected, result);
}
...
TEST_F(ArrayTopNTest, transformNZero) {
  ...
  auto result =
      evaluate("array_top_n(c0, BIGINT '0', x -> x)", makeRowVector({input}));
  assertEqualVectors(expected, result);
}
...
TEST_F(ArrayTopNTest, transformNLargerThanArray) {
  ...
  auto result =
      evaluate("array_top_n(c0, BIGINT '10', x -> x)", makeRowVector({input}));
  assertEqualVectors(expected, result);
}
...
TEST_F(ArrayTopNTest, transformStringsByLength) {
  ...
  auto result = evaluate(
      "array_top_n(c0, BIGINT '2', x -> length(x))", makeRowVector({input}));
  assertEqualVectors(expected, result);
}
...
TEST_F(ArrayTopNTest, transformNegativeN) {
  ...
  VELOX_ASSERT_THROW(
      evaluate("array_top_n(c0, BIGINT '-1', x -> x)", makeRowVector({input})),
...
TEST_F(ArrayTopNTest, transformReturnsNull) {
  ...
  auto result = evaluate(
      "array_top_n(c0, BIGINT '3', x -> if(x % 2 = 0, cast(null as integer), x))",
      makeRowVector({input}));
  assertEqualVectors(expected, result);
}
...
TEST_F(ArrayTopNTest, transformComplexExpression) {
  ...
  auto result = evaluate(
      "array_top_n(c0, BIGINT '3', x -> 0 - abs(x - 3))",
      makeRowVector({input}));
